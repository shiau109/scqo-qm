...
def test_the_dac_ceiling_agrees_with_quam_config_instrument_limits():
    ...
    and invisible in the simulator.
    """
...
def test_the_mw_family_is_bounded_in_a_different_UNIT_entirely():
    """Not a different number for the same thing: an MW-FEM amplitude is
    dimensionless and scaled by full_scale_power_dbm, an Octave one is volts at
    the DAC. Anything that treated the two ceilings as comparable would be wrong
    by whatever the full scale happens to be."""
    from types import SimpleNamespace as NS

    from quam_config.instrument_limits import instrument_limits

    mw = instrument_limits(NS(opx_output=NS(band=2, full_scale_power_dbm=-11)))
    assert mw.max_wf_amplitude == 1
    assert "full_scale_power_dbm" in mw.units
