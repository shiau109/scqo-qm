...
rr_freq = np.array([4.91225,4.8]) * u.GHz
rr_LO = 4.950 * u.GHz
...
xy_freq = np.array([3.078604778, 3.0]) * u.GHz
xy_LO = np.array([3.1, 3.1]) * u.GHz
...
for k, q in enumerate(machine.qubits):
    # readout
    machine.qubits[q].resonator.operations["readout"].length = 2.0 * u.us
